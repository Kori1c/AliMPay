...
- 首次登录后立即修改后台密码
- 如果要使用纯 Passkey 模式，请先注册至少一个 Passkey，并确认可以正常登录后再关闭密码登录
- 保存支付宝参数后先执行“测试连接”
- 选择正确的收款模式
...
- 生产环境建议全站使用 HTTPS
- Passkey 依赖浏览器 WebAuthn 能力，除 `localhost` 外通常需要 HTTPS 才能使用
...
诊断信息会自动脱敏，不会导出支付宝私钥、商户密钥、后台密码、Passkey 凭据或订单数据库。
